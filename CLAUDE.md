...
When the user asks to implement a feature or bugfix with the `auto-dev workflow`, Claude Code must follow:
`.agents/skills/auto-dev-workflow/references/claude-code-adapter.md`

For that workflow:
- do not edit the main `wcq` worktree directly
- use the repository's `.sh` workflow scripts for worktree setup, checks, commits, and merge
- if the user explicitly says `skip-workflow`, bypass this route and handle the request normally
