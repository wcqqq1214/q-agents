...
Python 3.13、LangChain、LangGraph で構築されたマルチエージェント金融分析システム。Fan-out / Fan-in トポロジーを採用し、Quant・News・Social の3エージェントが並列実行され、最終的に CIO エージェントが投資推奨をまとめます。リポジトリには FastAPI バックエンド、Next.js フロントエンド、実行ごとのレポート成果物、さらに任意で使える Redis / ARQ バックグラウンドジョブ基盤も含まれます。

## 機能

- **マルチエージェントアーキテクチャ**: Quant / News / Social エージェントの並列実行と CIO による統合
- **株式 + 暗号資産ワークスペース**: 株価は MCP 経由、暗号資産はホットキャッシュ経由で取得し、両アセット向けの OHLC チャート API を提供
- **可読レポートバンドル**: 各実行で `data/reports/{run_id}_{asset}/` に `report.json`、各エージェント JSON、可読 markdown を保存
- **ニュースインテリジェンス**: 複数ソース集約（DuckDuckGo、Tavily）とセンチメント分析
- **ソーシャルセンチメント**: Reddit ディスカッション分析による個人投資家心理の把握
- **ML シグナルガバナンス**: LightGBM、SHAP、銘柄別 OOS 指標、類似度ベースのシグナルフィルタリング
...
- **バックエンド**: `fastapi`, `uvicorn`, `httpx`, `apscheduler`
- **キュー / キャッシュ**: `redis`, `arq`（任意）
...
| `MINIMAX_API_KEY` | [MiniMax Platform](https://www.minimaxi.com/platform) | 必須 |
| `TAVILY_API_KEY` | [Tavily](https://tavily.com/) | 必須 |
| `POLYGON_API_KEY` | [Polygon.io](https://polygon.io/) | 任意 |
| `OKX_DEMO_API_KEY` / `OKX_DEMO_SECRET_KEY` / `OKX_DEMO_PASSPHRASE` | OKX | 任意 |

よく使うオプション設定: `CLAUDE_BASE_URL`、`CLAUDE_MODEL`、`MINIMAX_BASE_URL`、`MINIMAX_EMBEDDING_MODEL`、`MCP_MARKET_DATA_URL`、`MCP_NEWS_SEARCH_URL`、`USE_PROXY`、`REDIS_ENABLED`、`REDIS_URL`、`NEXT_PUBLIC_API_URL`。

設定 API/UI には互換性のため `OPENAI_API_KEY` フィールドも残っていますが、現在の埋め込みパイプラインが参照するのは `.env` の `MINIMAX_*` 設定です。

### 4. 任意: バックグラウンドジョブ用 Redis を起動

```bash
docker compose up -d
```

Redis がなくても、バックエンドは定期/バックグラウンド処理をプロセス内実行にフォールバックします。
...
| サービス | URL |
|---------|-----|
| フロントエンド（Home） | http://localhost:3000 |
| フロントエンド Reports | http://localhost:3000/reports |
| フロントエンド System | http://localhost:3000/system |
| フロントエンド Settings | http://localhost:3000/settings |
| API | http://localhost:8080 |
| API ドキュメント（Swagger）| http://localhost:8080/docs |

ホーム画面では株式と暗号資産のウォッチリストを切り替え、OHLC チャートを確認し、分析クエリを送信できます。結果は SSE で返され、完了した実行は `data/reports/{run_id}_{asset}/` に保存されます。
...
| `compare_models.py` | 単一銘柄向け markdown ML 評価レポートを生成 |
...
Redis を使うバックグラウンドジョブ、たとえば定期 OHLC 更新用の任意 ARQ ワーカーを起動します。
...
- `app/api/routes/reports.py` — 保存済みレポート一覧と詳細
- `app/api/routes/system.py` — MCP ヘルス / ステータス
- `app/api/routes/settings.py` — 実行時設定エンドポイント
- `app/api/routes/stocks.py` — 株式データエンドポイント
- `app/api/routes/ohlc.py` — 株式 OHLC 履歴とメタデータ
- `app/api/routes/crypto.py` — 暗号資産エンドポイント
- `app/api/routes/crypto_klines.py` — ホット/コールド統合の暗号資産 K 線
...
- `app/ml/model_registry.py` — モデル編成、比較レポート、スコア補助
- `app/ml/feature_engine.py` — 特徴量エンジニアリングパイプライン
- `app/ml/features.py` — テクニカル指標特徴量
- `app/ml/signal_filter.py` — 類似度ベースのシグナルガバナンス
- `app/ml/similarity.py` — 過去類似ケース検索
- `app/ml/text_features.py` — テキスト由来特徴量
- `app/ml/shap_explainer.py` — SHAP 説明可能性

### バックグラウンドジョブとリアルタイムデータ
- `app/services/realtime_agent.py` — 暗号資産ホットキャッシュのウォームアップと更新ループ
- `app/tasks/update_ohlc.py` — 日次 OHLC 更新タスク
- `app/tasks/worker_settings.py` — ARQ ワーカー設定
...
- `frontend/src/lib/api.ts` — 型付きフロントエンド API クライアント
...
- [docs/PROXY_CONFIGURATION.md](docs/PROXY_CONFIGURATION.md) — WSL2 / Linux / macOS / Windows 向けプロキシ設定
- [docs/okx-api-guide.md](docs/okx-api-guide.md) — OKX API 設定とエンドポイントガイド
