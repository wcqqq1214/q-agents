...
async def place_order(request: PlaceOrderRequest) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"Placing order - mode: {request.mode}, inst_id: {request.inst_id}, side: {request.side}, type: {request.order_type}")
...
async def get_order_details(
    ...
    """
    try:
        logger.info(f"Getting order details - mode: {mode}, order_id: {order_id}, inst_id: {inst_id}")
        client = get_okx_client(mode)
...
async def get_order_history(
    mode: str = Query(default="demo", description="Trading mode: demo or live"),
    inst_type: str = Query(default="SPOT", description="Instrument type (e.g., SPOT, SWAP, FUTURES)"),
    ...
    """
    try:
        logger.info(f"Getting order history - mode: {mode}, inst_type: {inst_type}, inst_id: {inst_id}, limit: {limit}")
        client = get_okx_client(mode)
