...
4. The stock quote cache TTL is raised to 5 minutes so frontend polling cadence and upstream cache cadence match.
...
Behavior:
- Raise `QUOTE_CACHE_TTL` from 60 seconds to 300 seconds.
...
- Continue fetching recent daily OHLC and overwriting same-day rows in the database.
...
4. Frontend repeats polling every 5 minutes while the document is visible.
...
5. Frontend re-fetches every 5 minutes while visible, so the latest partial daily candle appears without page reload.
...
- Raising quote cache TTL to 300 seconds prevents a 5 minute frontend cadence from repeatedly bypassing cache.
...
- Red-green test for quote cache TTL behavior where repeated requests within 300 seconds reuse cached data.
...
- Stock quote cache TTL is 300 seconds.
