...
export function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  const renderMarkdown = (text: string): string => {
    ...
    html = html.replace(/^## (.+)$/gim, '<h2 class="text-lg font-bold mt-5 mb-3 text-foreground">$1</h2>');
    html = html.replace(/^# (.+)$/gim, '<h1 class="text-xl font-bold mt-6 mb-4 text-foreground">$1</h1>');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('|')) {
        if (!inTable) {
          inTable = true;
          tableHtml = '<table class="w-full border-collapse my-3 text-sm"><tbody>';
        }
