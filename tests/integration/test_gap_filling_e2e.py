...
from app.database.crypto_ohlc import (
    get_crypto_ohlc,
    get_max_date,
    get_max_timestamp,
    upsert_crypto_ohlc,
...
async def test_gap_filling_end_to_end():
    ...
    old_data = [
        {
            ...
            "date": "2026-03-21T23:59:00+00:00",
            "open": 100,
            "high": 110,
            "low": 90,
            "close": 105,
    ...
    with (
        patch("app.services.realtime_agent.datetime") as mock_datetime,
        patch(
            "app.services.binance_client.fetch_klines_with_pagination",
            new_callable=AsyncMock,
        ) as mock_fetch_pagination,
        patch(
            "app.services.binance_client.fetch_binance_klines", new_callable=AsyncMock
        ) as mock_fetch_klines,
    ...
    for i in range(len(all_timestamps) - 1):
        ...
        if gap != 60000:
            gaps_found.append(
                {"from": all_timestamps[i], "to": all_timestamps[i + 1], "gap_ms": gap}
            )
