...
from app.okx.exceptions import OKXAuthError, OKXConfigError, OKXError, OKXRateLimitError
...
class TestAccountBalanceRoute:
    ...
    def test_get_balance_config_error(self, client):
        ...
        with patch(
            "app.api.routes.okx.get_okx_client",
            side_effect=OKXConfigError("Missing API key"),
        ):
            response = client.get("/api/okx/account/balance")
    ...
    def test_get_balance_invalid_mode(self, client):
        ...
        with patch(
            "app.api.routes.okx.get_okx_client",
            side_effect=OKXConfigError("Invalid mode: invalid"),
        ):
...
class TestAccountPositionsRoute:
    ...
    def test_get_positions_auth_error(self, client, mock_okx_client):
        ...
        mock_okx_client.get_positions.side_effect = OKXAuthError("Invalid signature", code="50113")
    ...
    def test_get_positions_config_error(self, client):
        ...
        with patch(
            "app.api.routes.okx.get_okx_client",
            side_effect=OKXConfigError("Missing config"),
        ):
            response = client.get("/api/okx/account/positions")
